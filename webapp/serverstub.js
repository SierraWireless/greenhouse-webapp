...
function getDataResponse() {
  ...
  var datas = {
    ...
    "greenhouse.data.light": [{
      "value": toogleLight.toString(),
      "timestamp": timestamp
    }],
    "greenhouse.data.open": [{
      "value": toogleShield.toString(),
      "timestamp": timestamp
    }],
...
function getCommStatus() {
  if (responseCount % 4 === 1) {
    ...
    return "ERROR";
  } else if (responseCount % 4 === 0) { return "UNDEFINED"; }
}
...
function handlePostRequest(postdata) {
  if (postdata !== null) {
    ...
    } else if (commandId === "greenhouse.data.switchShield") { return getCommandShieldResponse(postdata.paramValues[0].value); }
  }
